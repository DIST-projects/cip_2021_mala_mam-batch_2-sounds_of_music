...
def adjust_photo(image):
    ...
    if SAVING_IMAGES_STEPS:
        cv2.imwrite("C:\\cip\\music\\music\\static\\media\\output\\1a_cvtColor.jpg", gray)
        cv2.imwrite("C:\\cip\\music\\music\\static\\media\\output\\1b_GuassianBlur.jpg", blur)
        cv2.imwrite("C:\\cip\\music\\music\\static\\media\\output\\1c_canny.jpg", edged)
    ...
    cv2.imwrite("C:\\cip\\music\\music\\static\\media\\output\\2with_contours.png", image)
    dst = cv2.cvtColor(dst, cv2.COLOR_BGR2GRAY)

    _, result = cv2.threshold(dst, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    if SAVING_IMAGES_STEPS:
        cv2.imwrite("C:\\cip\\music\\music\\static\\media\\output\\3adjusted_photo.png", result)
    return result
