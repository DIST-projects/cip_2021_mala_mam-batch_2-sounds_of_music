...
def get_clef(image, staff):
    ...
    if SAVING_IMAGES_STEPS:
        cv2.imwrite("C:\\cip\\music\\music\\static\\media\\output\\7clef.png", window)
    return window
...
def hu_moments():
    """
    Computes hu moments 
    :return: staff clef hu moments
    """
    violin_key = cv2.imread("C:\\cip\\music\\music\\static\\media\\clef_samples\\violin_clef.png", 0)
    bass_key = cv2.imread("C:\\cip\\music\\music\\static\\media\\clef_samples\\bass_clef2.png", 0)
...
def classify_clef(image, staff):
    """
    Uses Hu moments to classify the clef.
