...
def detect_blobs(input_image, staffs):
    ...
    if SAVING_IMAGES_STEPS:
        cv2.imwrite("C:\\cip\\music\\music\\static\\media\\output\\8a_lines_horizontal_removed.png", horizontal_lines)
        cv2.imwrite("C:\\cip\\music\\music\\static\\media\\output\\8a_lines_vertical_removed.png", vertical_lines)
    ...
    if SAVING_IMAGES_STEPS:
        cv2.imwrite("C:\\cip\\music\\music\\static\\media\\output\\8b_with_blobs.jpg", im_with_blobs)
    ...
    for idx, tup in enumerate(sorted_notes):
        ...
        cv2.putText(im_with_blobs, str(tup[1]), (int(tup[0].pt[0]), int(tup[0].pt[1])),
                    ...
                    fontScale=1, color=(255, 0, 0))
    if SAVING_IMAGES_STEPS:
        cv2.imwrite("C:\\cip\\music\\music\\static\\media\\output\\8c_with_numbers.jpg", im_with_numbers)
        cv2.imwrite("C:\\cip\\music\\music\\static\\media\\output\\8d_with_staff_numbers.jpg", im_with_blobs)
